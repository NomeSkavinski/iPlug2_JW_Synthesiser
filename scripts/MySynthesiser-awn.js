/* Declares the MySynthesiser Audio Worklet Node */

class MySynthesiserController extends WAMController
{
  constructor (actx, options) {
    ...
    super(actx, "MySynthesiser", options);
  }

  static importScripts (actx) {
    var origin = "https://NomeSkavinski.github.io/iPlug2_JW_Synthesiser/";

    return new Promise( (resolve) => {
      actx.audioWorklet.addModule(origin + "scripts/MySynthesiser-wam.js").then(() => {
      actx.audioWorklet.addModule(origin + "scripts/wam-processor.js").then(() => {
      actx.audioWorklet.addModule(origin + "scripts/MySynthesiser-awp.js").then(() => {
        resolve();
      }) }) });
    })
  }
