...
if %DEMO_ARG% == "demo" (
  echo Making MySynthesiser Windows DEMO VERSION distribution ...
  set DEMO=1
) else (
  echo Making MySynthesiser Windows FULL VERSION distribution ...
  set DEMO=0
)
...
REM - msbuild MySynthesiser-app.vcxproj /p:configuration=release /p:platform=win32

REM echo Building 32 bit binaries...
REM msbuild MySynthesiser.sln /p:configuration=release /p:platform=win32 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly 

REM echo Building 64 bit binaries...
REM add projects with /t to build VST2 and AAX
msbuild MySynthesiser.sln /t:MySynthesiser-app;MySynthesiser-vst3 /p:configuration=release /p:platform=x64 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly;append
...
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\MySynthesiser.aaxplugin\Contents\Win32\MySynthesiser.aaxplugin --out .\build-win\aax\bin\MySynthesiser.aaxplugin\Contents\Win32\MySynthesiser.aaxplugin
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\MySynthesiser.aaxplugin\Contents\x64\MySynthesiser.aaxplugin --out .\build-win\aax\bin\MySynthesiser.aaxplugin\Contents\x64\MySynthesiser.aaxplugin
...
echo Making Installer ...
  ...
  REM :32-Bit-is
  REM REM "%ProgramFiles%\Inno Setup 6\iscc" /Q ".\installer\MySynthesiser.iss"
  REM goto END-is

  REM :64-Bit-is
  "%ProgramFiles(x86)%\Inno Setup 6\iscc" /Q ".\installer\MySynthesiser.iss"
  REM goto END-is

  REM :END-is

  REM - Codesign Installer for Windows 8+
  REM -"C:\Program Files (x86)\Microsoft SDKs\Windows\v7.1A\Bin\signtool.exe" sign /f "XXXXX.p12" /p XXXXX /d "MySynthesiser Installer" ".\installer\MySynthesiser Installer.exe"

  REM -if %1 == 1 (
  REM -copy ".\installer\MySynthesiser Installer.exe" ".\installer\MySynthesiser Demo Installer.exe"
  REM -del ".\installer\MySynthesiser Installer.exe"
  REM -)
