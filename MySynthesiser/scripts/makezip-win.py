...
def main():
  ...
  if not zip:
    installer = "\\build-win\\installer\\MySynthesiser Installer.exe"
    
    if demo:
      installer = "\\build-win\\installer\\MySynthesiser Demo Installer.exe"
    
    files = [
      projectpath + installer,
      projectpath + "\installer\changelog.txt",
      projectpath + "\installer\known-issues.txt",
      projectpath + "\manual\MySynthesiser manual.pdf" 
    ]
  else:
    files = [
      projectpath + "\\build-win\MySynthesiser.vst3\Contents\\x86_64-win\MySynthesiser.vst3",
      projectpath + "\\build-win\MySynthesiser_x64.exe"  
    ]

  zipname = "MySynthesiser-v" + config['FULL_VER_STR']
  ...
  files = [
    projectpath + "\\build-win\\pdbs\\MySynthesiser-vst3_x64.pdb",
    projectpath + "\\build-win\\pdbs\\MySynthesiser-app_x64.pdb"  
  ]
